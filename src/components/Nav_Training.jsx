...
const Nav_Training = ({ styles = "" }) => {
  return (
    <div className={`bg-gradient-to-r from-slate-950/95 via-black to-slate-950/95 text-white border-t border-white/10 backdrop-blur-md w-full ${styles}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> {/* ✅ REMOVED w-screen */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 lg:gap-6 py-6 justify-items-center">
          {navOption.map((option, index) => (
            <a key={index} href="#" className="group text-center transition-all duration-300 hover:text-amber-400 hover:-translate-y-0.5 cursor-pointer block w-full">
