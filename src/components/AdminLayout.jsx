...
const AdminLayout = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      await axios.post("http://localhost:4000/api/v1/admin/logout", {}, {
        withCredentials: true
      });
    } catch (error) {
      console.log("Logout error:", error);
    } finally {
      // 2. CRITICAL FIX: Destroy the frontend token so the Bouncer knows you are gone
      localStorage.removeItem("adminToken");
      
      // 3. Send back to the login screen
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside className={`fixed lg:static inset-0 z-40 bg-white shadow-lg transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform w-64 h-full`}>
