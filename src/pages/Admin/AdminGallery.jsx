...
const AdminGallery = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:4000/api/v1/admin/gallery/upload",
        uploadData,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(
        `http://localhost:4000/api/v1/admin/gallery/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="bg-white dark:bg-[#1f2937] rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8 transition-colors duration-300">
        ...
        >
          ...
          <button
            type="submit"
            disabled={uploading}
            className="px-6 py-3 rounded-2xl bg-[#556b2f] hover:bg-[#6b8637] text-white font-medium transition"
          >
