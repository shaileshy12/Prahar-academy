...
function useLangPrefix() {
  ...
  const currentLang = SUPPORTED_LANGS.includes(maybeLang) ? maybeLang : DEFAULT_LANG;
...
function Header() {
  ...
  const toggleDesktopMenu = (name) => setOpenDesktopMenu((prev) => (prev === name ? null : name));
  const closeAllMenus = () => { setOpenMobileNav(false); setOpenDesktopMenu(null); setOpenSection(null); };

  return (
    <header key={i18n.language} className="w-full bg-white border-b border-gray-200 relative z-30">
      <div className="mx-auto px-3 sm:px-4 lg:px-6">
        <div className="flex items-center justify-between gap-2 py-2 sm:py-3">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-sm sm:text-base md:text-lg lg:text-xl font-bold tracking-wide text-gray-900 leading-tight truncate">
                {t("site.name")} <span className="hidden sm:inline">(Borivali)</span>
              </span>
          ...
          <div className="hidden lg:flex flex-col items-end gap-2 text-xs xl:text-sm">
            <div className="flex flex-wrap items-center justify-end gap-x-3 gap-y-1 text-gray-800">
              <span className="inline-flex items-center gap-1">
                ...
                <a href={`mailto:${t("header.email")}`} className="hover:underline">
                  {t("header.email")}
                </a>
              </span>
              <span className="inline-flex items-center gap-1">
          ...
          <div className="flex items-center gap-2 md:hidden">
            <a href="tel:+919594801555" className="text-orange-700 p-1.5">
            ...
            >
              {openMobileNav ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>
          </div>
        </div>
          ...
          >
            <nav className="flex flex-wrap items-center justify-between gap-2 px-4 py-1.5">
              ...
              <div className="flex flex-wrap justify-center gap-3 lg:gap-5 text-sm lg:text-[15px]">
                <Link to={refArr["Home"]} className="px-1 hover:text-yellow-200 transition">
                  {t("nav.home")}
                </Link>
                  ...
                  {openDesktopMenu === "about" && (
                    <div className="absolute left-0 mt-2 w-44 rounded-lg bg-white text-black shadow-xl z-40">
                      <div className="flex flex-col py-2 text-sm">
                        <Link to={refArr["About Us"]} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          {t("nav.about")}
                        </Link>
                        <Link to={prefix + "/director-desk"} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          Director Desk
                        </Link>
                        <Link to={prefix + "/mission"} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          Mission
                        </Link>
                        <Link to={prefix + "/vision"} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          Vision
                        </Link>
                      </div>
                    </div>
                ...
                <Link to={refArr["Success Student"]} className="px-1 hover:text-yellow-200 transition">
                  {t("nav.success")}
                </Link>
                ...
                <div className="relative">
                  ...
                  {openDesktopMenu === "gallery" && (
                    <div className="absolute left-0 mt-2 w-40 rounded-lg bg-white text-black shadow-xl z-40">
                      <div className="flex flex-col py-2 text-sm">
                        <Link to={prefix + "/gallery#photos"} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          Photos
                        </Link>
                        <Link to={prefix + "/gallery#videos"} onClick={() => setOpenDesktopMenu(null)} className="px-4 py-1 hover:bg-orange-50 hover:text-orange-700">
                          Videos
                        </Link>
                      </div>
                    </div>
                ...
                <Link to={refArr["Enquiry"]} className="px-1 hover:text-yellow-200 transition">
                  {t("nav.enquiry")}
                </Link>

                <Link to={refArr["Contact Us"]} className="px-1 hover:text-yellow-200 transition">
                  {t("nav.contact")}
                </Link>
              </div>
            </nav>
      ...
      >
        <div className="absolute inset-0 bg-black/70" onClick={() => setOpenMobileNav(false)} />
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
            ...
            <button type="button" onClick={() => setOpenMobileNav(false)} className="text-white">
              <X className="w-5 h-5" />
            </button>
          </div>

          <nav className="flex flex-col px-4 py-4 text-sm font-semibold space-y-1">
            <Link to={refArr["Home"]} onClick={closeAllMenus} className="py-2 text-yellow-300">
            ...
            <div className={`overflow-hidden transition-[max-height] duration-300 ${openSection === "about" ? "max-h-64" : "max-h-0"}`}>
            ...
            >
              ...
              {openSection === "courses" ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </button>

            <div className={`overflow-hidden transition-[max-height] duration-300 ${openSection === "courses" ? "max-h-80" : "max-h-0"}`}>
              <div className="flex flex-col pl-4 pb-2 gap-2 text-xs">
            ...
            <Link to={refArr["Success Student"]} onClick={closeAllMenus} className="py-2 border-t border-white/10">
            ...
            >
              ...
              {openSection === "gallery" ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
            </button>

            <div className={`overflow-hidden transition-[max-height] duration-300 ${openSection === "gallery" ? "max-h-40" : "max-h-0"}`}>
              <div className="flex flex-col pl-4 pb-2 gap-2 text-xs">
            ...
            <Link to={refArr["Enquiry"]} onClick={closeAllMenus} className="py-2 border-t border-white/10">
              {t("nav.enquiry").toUpperCase()}
            </Link>

            <Link to={refArr["Contact Us"]} onClick={closeAllMenus} className="py-2 border-t border-b border-white/10">
